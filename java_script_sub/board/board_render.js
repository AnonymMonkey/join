...
function generateTasksHTML(element) {
  ...
  return /*html*/ `
  <div id="${element["id"]}" draggable="true" onclick="generateOverlayContent(${element["id"]}),openTaskOverlay()" onmousedown="startTransform(${
    element["id"]
  }, '${status}')" onmouseup="stopTransform(${element["id"]}, '${status}')" ondragstart="startDragging(${element["id"]})"  class="card pointer">
    <div class="frame119">
      <div class="card_headline">
      <div class="board_card" style="background: ${taskCategory[element["category"]]["bgColor"]}">
      <span class="board_card_label">${taskCategory[element["category"]]["title"]}</span></div><div class="card_actions" onclick="doNotClose(event)">
...
function generateOverlayContent(element) {
  ...
  <div class="frame203_task">                   
    <div class="board_card_task" style="background: ${taskCategory[tasks[id]["category"]]["bgColor"]}">
      <span class="board_card_label_task">${taskCategory[tasks[id]["category"]]["title"]}</span>
    </div>  
