...
function renderUsersInUserselection(select, search, isClosed) {
  ...
  for (let contact of contacts) {
    let { contact_name: name, contact_initials: initials, contact_color: color, contact_ID: ID } = contact;
...
function createBadge(ID) {
  ...
  for (let i = 0; i < contactSelection.length; i++) {
    ...
    let initials = getIndexOfJson("contact_ID", selection)["contact_initials"];
    let color = getIndexOfJson("contact_ID", selection)["contact_color"];
